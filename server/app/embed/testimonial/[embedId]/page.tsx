...
import { WallDesign2 } from "@/components/testimonial-walls/wall-design-2"
import { WallDesign3 } from "@/components/testimonial-walls/wall-design-3"
...
export default function EmbedTestimonialPage() {
  ...
  const [testimonialWall, setTestimonialWall] = useState<TestimonialWall | null>(null)
  ...
  const fetchTestimonialWall = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/testimonial-walls/embed/${embedId}`)
      if (response.ok) {
  ...
  if (error || !testimonialWall) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-muted-foreground">{error || "Testimonial wall not found"}</p>
      </div>
    )
  }
  ...
  const renderWall = () => {
    ...
    switch (design) {
      case "2":
        return <WallDesign2 testimonials={testimonialWall.feedbacks} title={title} />
      case "3":
        return <WallDesign3 testimonials={testimonialWall.feedbacks} title={title} />
      default:
        return <WallDesign1 testimonials={testimonialWall.feedbacks} title={title} />
    }
  }
