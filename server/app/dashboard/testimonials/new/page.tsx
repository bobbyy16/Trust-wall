...
export default function NewTestimonialWallPage() {
  ...
  const fetchApprovedFeedbacks = async (widgetId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/testimonial-walls/approved-feedbacks/${widgetId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/testimonial-walls", {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen bg-muted/20">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Select approved feedbacks to display in your testimonial wall</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Choose which widget's feedbacks to display</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {selectedWidget && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Choose which approved feedbacks to include in your testimonial wall</CardDescription>
                </CardHeader>
                <CardContent>
                  {feedbacks.length === 0 ? (
                      ...
                      {feedbacks.map((feedback) => (
                        <div key={feedback.id} className="flex items-start space-x-3 p-4 border rounded-lg">
                          <Checkbox
                            ...
                            onCheckedChange={() => handleFeedbackToggle(feedback.id)}
                          />
            ...
            <div className="flex gap-4">
              <Button type="submit" disabled={isLoading || !selectedWidget || selectedFeedbacks.length === 0}>
