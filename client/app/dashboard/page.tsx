  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const approvedFeedbacks =
        feedbacksData.feedbacks?.filter((f: Feedback) => f.approved).length ||
        0;
      const averageRating =
        totalFeedbacks > 0
          ? feedbacksData.feedbacks.reduce(
  ...
  const approveFeedback = async (feedbackId: string, approved: boolean) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/feedbacks/${feedbackId}/approval`,
        {
          method: "PATCH",
          headers: getAuthHeaders(),
      ...
      if (response.ok) {
        setFeedbacks(
          feedbacks.map((f) => (f.id === feedbackId ? { ...f, approved } : f))
        );
        toast({
          title: "Success",
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {stats.totalFeedbacks > 0
                  ? Math.round(
                      (stats.approvedFeedbacks / stats.totalFeedbacks) * 100
                    )
                  : 0}
                % approval rate
              </p>
            ...
            {widgets.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">
                  Create your first widget to start collecting customer
                  feedback.
                </p>
                <Button asChild>
                  <Link href="/dashboard/widgets/new">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => copyEmbedCode(widget.embedId)}
                        >
                        ...
                        <div className="flex space-x-2">
                          <Button size="sm" variant="outline" asChild>
                            <Link
                              href={`/embed/widget/${widget.embedId}`}
                              target="_blank"
                            >
          ...
          <TabsContent value="feedback" className="space-y-6">
            ...
            {feedbacks.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Share your widget embed code to start collecting customer
                  feedback.
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center space-x-2">
                            ...
                            <Badge
                              variant={
                                feedback.approved ? "default" : "secondary"
                              }
                            >
            ...
            {testimonialWalls.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  No testimonial walls yet
                </h3>
                <p className="text-muted-foreground mb-4">
                  Create your first testimonial wall to showcase approved
                  feedback.
                </p>
                <Button asChild>
                  <Link href="/dashboard/testimonials/new">
